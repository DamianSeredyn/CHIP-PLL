...
Kolumny w plikach danych (wrdata):
  time  v(vout)  v(x1.biasn)  i(v.x1.Viup)  i(v.x1.Vidn)
"""
...
def parse_tag(filename: str):
    ...
    # fallback – generyczne parsowanie
...
def read_data(filepath: str):
    """
    Wczytuje plik tekstowy ngspice (wrdata) i zwraca pięć tablic:
      time, vout, biasn, i_viup, i_vidn

    Netlistа zawiera:
      wrdata cp_test.txt v(vout) v(x1.biasn) i(v.x1.Viup) i(v.x1.Vidn)

    ngspice wrdata zapisuje każdy wektor jako parę (skala_czasu, wartość),
    więc dla 4 sygnałów mamy 8 kolumn:
      col0=time  col1=vout
      col2=time  col3=biasn
      col4=time  col5=i_viup
      col6=time  col7=i_vidn
    """
    rows = []
...
for fpath in data_files:
    ...
    try:
        time, vout, biasn, i_viup, i_vidn = read_data(fpath)
    ...
    i_viup_nA = i_viup * 1e9   # A → nA
    i_vidn_nA = i_vidn * 1e9   # A → nA
    ...
    # Panel 3 – Prąd Viup
    axes[2].plot(time_us, i_viup_nA, color=COLORS['i_viup'], linewidth=1.2)
    axes[2].axhline(avgs['i_Viup'], color=COLORS['i_viup'], linestyle='--',
                    linewidth=0.8, alpha=0.6,
                    label=f"śr. = {avgs['i_Viup']:.4f} nA")
    axes[2].set_ylabel('i(Viup) [nA]')
    axes[2].set_title('Prąd źródła Viup')
    axes[2].legend(fontsize=9, loc='upper right')
    axes[2].grid(True, alpha=0.3)

    # Panel 4 – Prąd Vidn
    axes[3].plot(time_us, i_vidn_nA, color=COLORS['i_vidn'], linewidth=1.2)
    axes[3].axhline(avgs['i_Vidn'], color=COLORS['i_vidn'], linestyle='--',
                    linewidth=0.8, alpha=0.6,
                    label=f"śr. = {avgs['i_Vidn']:.4f} nA")
    axes[3].set_ylabel('i(Vidn) [nA]')
...
for tag, corner, temp, vp, avgs in summary:
    if corner != current_corner:
        ...
        summary_rows += (
            ...
            f'<td colspan="7"><b>Corner: {corner} ({corner_label})</b></td>'
            f'</tr>\n'
        )
    summary_rows += f'''
        <tr>
